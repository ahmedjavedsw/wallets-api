...
class WalletUS_Portal_Service
{
    private $gateway_url = "https://walletes.com/";
    private $params = [];
    private $args = [];
    public $http_code = 0;
    ...
    public function generate_link() {
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $this->gateway_url.'generate-dhru-order');
...
function walletusportal_config()
{
    $configarray = array(
        'name' => array(
            'Type' => 'System',
            'Value' => 'WalletUs Portal - Auto Crypto Payments USDT & Binance'
        ),
        'email' => array(
            'Name' => 'WalletUs Account Email',
            'Type' => 'text',
            'Size' => '50',
            'Description' => '<a href="https://walletes.com/register" target="_blank" style="color:blue;">Register</a>'
        ),
        'api_key' => array(
            'Name' => 'API Key',
            'Type' => 'text',
            'Value' => '',
            'Size' => '32',
            'Description' => '<a href="https://walletes.com/client" target="_blank" style="color:blue;">Get Your Api Key</a>'
        ),
...
function walletustportal_link($params)
{   
    global $lng_languag;
    $client = new WalletUS_Portal_Service($params);
    $server_response = $client->generate_link();

    if($client->http_code === 403) {
        return '<p style="color:red;">Please whitelist your Server IP from <a style="color:#FF5555;" href="https://walletes.com/client" target="_blank" style="color:red;">Merchant Panel</a></p>';
    }

    if($client->http_code !== 200) {
        return '<p style="color:red;">WAllet Us Portal Server Offline</p>';
    }
